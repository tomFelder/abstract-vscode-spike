...
- **F1 RESOLUTION (desktop check, 10 Jul):** X1 does NOT reproduce on desktop - the approve path writes to disk and survives a full relaunch ([x1-desktop-check.md](x1-desktop-check.md)); the persist logic is correct and the web build's File System Access provider is the limitation. F1 therefore narrows to: (a) the web build's persistence/reload contract, and (b) a NEW real finding, **F19: the History tab does not rehydrate from the on-disk lock audit on a cold reopen** (data persists, the timeline renders empty) - a display-rehydration gap, Tier 1 severity, owned by plan 37 iteration 1 alongside F1.
