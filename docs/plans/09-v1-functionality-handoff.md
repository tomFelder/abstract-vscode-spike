...
visibly broken):
1. **Agentic loop is real.** A source/context change -> the agent proposes genuine figure *and*
   meaning changes (model-backed, not the "...revisit whether this still holds" heuristic) -> they land
   in the Review rail with model-quality old->new diffs -> approve applies / reject reverts. (Approve/
   reject plumbing already works - iter 7; the gap is the *model* behind the proposals.)
...
3. **Skills run for real.** Financial (deterministic, done) + Formatting (deterministic, done) +
   **Strategy** (model-backed: claims vs the Knowledge decision stack) all produce live verdicts;
   **Apply fix** actually edits the document; Run/Re-run work (done).
...
## The single biggest lever: wire a language model

Three of the seven criteria (agentic loop, Chat, Strategy skill) are **blocked on the absence of a
language model** in the web build. **Do this first** - it unblocks the most surface area:
- The service already has the hooks: `LivingDocsService._hasModel()` / `_proposeImpact` ->
  `_modelImpact` (falls back to `_heuristicImpact`), and `_gradeStrategy`. They use
  `ILanguageModelsService.selectLanguageModels(...)`. Today nothing registers a model, so `_hasModel()`
  is false and you get the honest heuristic/"NO MODEL" states.
- **Task:** register a working language-model provider for the web build (e.g. a built-in extension or
  a service that talks to the **Claude API** - see the `claude-api` skill for current model ids and the
  Anthropic SDK), wired behind a config/secret for the key. Then `_modelImpact` and `_gradeStrategy`
  produce real output. Keep the no-model path as a graceful fallback (it is honest and already tested).
- **Verify live:** change `market-research.md`, open the doc, Context panel -> **Review impact** ->
  you should now get a *real* commentary rewrite (e.g. tone/claim-aware), not the boilerplate note.
- **Brainstorm the model wiring with the user before building it** (key management, which provider,
  cost) - use `superpowers:brainstorming`. This is the one task worth a design conversation up front.

## Setup (unchanged from the audit)

- Repo `/Users/tommy/Sites/abstract-vscode-spike`. **Branch off the audit work** so you keep its gains:
  cut `living-docs-v1` off `living-docs-design-audit` (or merge PR #9 to `living-docs-design-match`
  first, then branch off that). Open ONE `living-docs-v1` PR; commit each iteration; PRs embed
...
1. **Wire a language model** (unblocks 3 criteria - do first; brainstorm with the user).
2. **Chat composer -> real agent** (input, @mentions, tool-call render, model reply).
3. **Strategy skill + Apply-fix** (model-backed grader; Apply fix edits the doc).
...
`/loop` with no interval self-paces (one iteration per turn). The very first iteration should be the
**language-model wiring brainstorm + spike** (criterion 1's blocker), since it unblocks the most.
