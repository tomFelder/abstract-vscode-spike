...
| [39-t1-editor-audit-loop.md](39-t1-editor-audit-loop.md) | **The T1 editor audit as a runnable loop** (paste into a fresh session): 8 graded areas (paste-from-Word first), Word-clipboard fixtures, evidence per probe, gating-vs-polish triage, verdict PROCEED/FIX-FIRST/DISQUALIFIED posted to #128. **Verdict: FIX-FIRST**, fix order now executed; import/export (#129/#130) unblocked. |
| [40-file-interop-and-gate-close-loop.md](40-file-interop-and-gate-close-loop.md) | **The next loop (parallel-agent dispatch).** Two tracks: the P1 file-interop door (docx→md import #129, docx/PDF export #130, xlsx→CSV + PDF sources #131 - doc 22 §2-4) and the buildable gate residual (X1 survivability #121). Parks the you-dependent gate items (#120 model backend, #134 PostHog cloud) as a founder checklist. Closes doc 22 §6 and doc 18 §2.3 R4. |
