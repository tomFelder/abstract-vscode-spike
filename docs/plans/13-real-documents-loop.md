...
| **R6** Reference another file via UI — Context "＋ Add context" picks a REAL file; @mention resolves real folder files | A file-picker in the Context add form; @mention resolution over the real folder | **DONE (iter 4)** — verified live | Add-context form gained a **File** kind (picker of folder md/csv/json via `getContextCandidates`) → `addContextFile` writes `context:` frontmatter; × unbind on Referenced files (`removeContextFile`). `getMentionableFiles` broadened to all real folder docs (cached `folderFiles`), so `@mention` resolves any folder file. TDD; 0 core patches. Verified live: referenced/unreferenced `Team Notes.md`; `@`-chips list `forecast.csv`/`Team Notes.md`. Decision #41. |
