...
| I5 | `mammoth@1.8.0` runtime dependency (pure-JS docx parser) | dependency | `package.json`, `package-lock.json` | Doc 22 section 2's recommended pipeline; `jszip` was already transitively present. Not a core patch; used only by the proxy at runtime (the TS build never imports it) |

**A1 docx import: 0 core patches.** The whole feature lives in the `livingDocs` contribution + our node proxy, plus one pure-JS npm dependency (mammoth) that only the proxy loads at runtime - so the merge tax is a single line in `package.json`, not a fork of any upstream file. Verified: `typecheck-client` 0; `valid-layers-check` 0 in changed files; the conversion pipeline exercised end-to-end against a real `.docx` through the live proxy (`docs/plans/40-verify/a1-import/`).
