...
### File-reality wave (#245) - sanctioned seam budget: 1

The dual-stack broker bind (PR #247, issue #121) is correct broker hardening but does not by itself make
the packaged desktop app reach the broker: the workbench renderer polls the local broker over plain http on
8090 (`DEFAULT_PROXY_URL = "http://localhost:8090"`), and the stock desktop CSP `connect-src 'self' https:
ws:` blocks that fetch before a socket opens, so the app shows "Model unavailable". The broker script is not
bundled in the packaged build (the supervisor logs "broker script not found under appRoot; skipping"), so
the renderer's direct fetch is the only path and CSP is the sole barrier. This is the ONE sanctioned core
seam of the wave, budgeted up front.

| Item | Change | Tier | File(s) | Note / re-pin check |
|------|--------|------|---------|---------------------|
| FR-1 | Extend the workbench renderer CSP `connect-src` with exactly the two loopback broker origins `http://localhost:8090 http://127.0.0.1:8090` so the packaged desktop app can reach its own local model broker. Nothing broader - no bare `http:`, no wildcard ports; ws origins are deliberately NOT added because the livingDocs service never opens a websocket or EventSource against the broker (all broker traffic - healthz, models, `/v1/messages` completions, docx import/export, sources, auth - is plain http request/response). | **core-patch** | `src/vs/code/electron-browser/workbench/workbench.html` (production), `src/vs/code/electron-browser/workbench/workbench-dev.html` (dev, kept in parity; it already carried upstream's wildcard `http://localhost:*` / `http://127.0.0.1:*` from the usage-based-billing change, so the two exact origins broaden nothing there) | The desktop app must reach its own loopback broker (P0 #121); model calls, docx import and docx export all die silently without it, and the app reports "Model unavailable". These are the only two files in the workbench renderer's broker fetch path: the sole other CSP-adjacent layer, the `session.webRequest.onBeforeRequest`/`onHeadersReceived` handlers in `electron-main/app.ts`, only validate `vscode-webview`/`vscode-file` schemes, block `.svg`, and add PRSS-CDN CORS - none rewrites CSP or cancels a plain `http://localhost:8090` request. The sessions window html hosts no livingDocs surface, so its CSP is not in this path and is left untouched. **Fail-soft:** the entries only ALLOW loopback fetches; dropping them degrades to "Model unavailable" (no crash, the renderer catches the failed fetch). **Re-pin (`check-seams.sh` seam 11 `csp-broker-origin`):** both html files must carry both exact origins in connect-src; a rebase that drops either line fails the seam loudly instead of silently re-breaking the packaged app's broker. |
...
### Backfill: plan 37 F2 (stock Copilot chat removal, landed in #153): 0 ADDED core patches

Recorded late. F2 landed on 14 Jul in `bb2b5821863` ("the cheap FRAGILE cures batch") but never got its own
ledger rows, so the fork's most rename-fragile deregistration was documented only obliquely inside row K2.
The two rows below describe what is actually in `main` today. **0 core patches** - the public
`deregisterViewContainer` and `KeybindingsRegistry` routes reached it with none, so this backfill changes no
count.

| # | Change | Tier | File(s) | Note / re-pin check |
|---|--------|------|---------|---------------------|
| 37-F2a | Deregister the stock GitHub Copilot chat container so the "Chat" tab beside the Abstract Review rail no longer opens the raw upstream "Build with Agent" / "sign in to use Copilot" panel (walk 1e X4). Adds `'workbench.panel.chat'` (`ChatViewContainerId`) to `IDE_VIEW_CONTAINER_IDS`, hidden by the existing `HideIdeContainersContribution` via the public `deregisterViewContainer` registry route - the same seam already used for Explorer/Search/SCM/Debug/Extensions | additive-contribution | `livingDocs/browser/livingDocs.contribution.ts` | **Joins the HIGH-risk "fails-unsafely on container-id rename" set:** if upstream renames `workbench.panel.chat` the deregistration silently no-ops and the stock chat tab returns. `chat.disableAIFeatures:true` (decision 55) strips the AI chrome but does NOT remove the container, so config alone cannot hold this. **Now guarded:** `check-seams.sh` seam 1 carries the id (added 28 Jul), so a rename fails the seam loudly instead of silently restoring the tab. No core edit. |
| 37-F2b | Neutralise the stock Copilot chat-open chord by shadowing it with the built-in `noop` command at weight 1000, the same additive `KeybindingsRegistry` route as the other IDE chords (plan 33 iter 3) | additive-contribution | `livingDocs/browser/livingDocs.contribution.ts` (`NEUTRALISED_IDE_CHORDS`) | Belt-and-braces: the container is already gone (37-F2a), so the chord would open nothing; this stops it trying. Pinned to upstream's real binding `CtrlCmd|Alt|KeyI` (mac `CtrlCmd|WinCtrl|KeyI`), matching `chatActions.ts`. Re-pin if the chord changes upstream. No core edit. |
