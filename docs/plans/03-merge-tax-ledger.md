...
| Item | Change | Tier | File(s) | Note / re-pin check |
|------|--------|------|---------|---------------------|
| V2-1 | **NOT TAKEN - 0 core (plan 44-a, PR #218).** The tree rail / editor / right rail read as floating cards (12px gaps, radius 14, `#E9EAEE` border, rail `#FBFCFD` / editor `#FFFFFF`, shadow-rail / shadow-editor + e1) on the `#EDEFF3` chrome - delivered **CSS-only**, no part-margins core seam taken. | styleOverrides-CSS + theme | `styleOverrides/browser/media/elevation.css` (new module, wired in `styleOverrides.contribution.ts`), `theme-defaults/themes/abstract-color-theme.json` (`sideBar.background` + section headers `#fbfcfd`) | The cards **ride the existing core floating-panels feature** (`browser/media/floatingPanels.css` + `AbstractPaneCompositePart.getFloatingInset`, which reserves `FLOATING_PANEL_MARGIN` so card content never clips - the inner-inset "fight the layout" trap is avoided entirely). elevation.css only re-skins those cards + repaints the grid backdrop to chrome; it does **not** position parts. Measured live (1440×900 + 1760×1000): 12px inter-card gaps, 12px left/right frame insets, radius 14, exact shadows, rail #FBFCFD / paper #FFFFFF, `.monaco-editor-background` pinned opaque `#FFFFFF !important`, sashes resize (252→312px drag verified). **Residual ±6px:** the top edge is flush under the (future 48px) header band and the bottom edge is floating-panels' 6px content-synced margin (CSS margins can't widen these without overflowing the fixed grid part; a 6px bottom gap is well within calm tolerance and not worth a second wave seam). Asserted by `check-seams.sh` seam 9 (elevation.css gates on `.floating-panels`; the core feature still exists; the chrome/border/shadow tokens stay pinned). Re-pin: if a rebase renames `.floating-panels` or drops the feature, seam 9 fails loud; re-key elevation.css to the new mechanism. |
