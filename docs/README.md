...
| [26-glossary-and-id-index.md](26-glossary-and-id-index.md) | **Keep open while reading anything else.** Every ID system (journeys, F-fixes, map-D vs decisions, VPs, T-tests, principle-P vs priority-P), each mapped to its home doc. The vocabulary itself is canonical in the root [CONTEXT.md](../CONTEXT.md) as of 1 Sep 2026 |
...
| [31-backlog-sequencing.md](31-backlog-sequencing.md) | **Why the backlog is in the order it is** (1 Sep 2026). The reasoning behind the seven milestones and the P0/P1/P2 scale set by the first full triage: what P0 is reserved for, why the tooling wave comes before the data-loss wave, and the cross-issue pairs that share one fix. Live state lives in the GitHub milestones and labels, not here |
...
| Doc | What it covers |
|---|---|
| [07-decision-log.md](07-decision-log.md) + [adr/](adr/README.md) | **The running ADR log: 154 decisions, numbered 1-181** (102-109, 112-119 and 139-149 were reserved and never used), from spike calls through the beta-planning, Editor-v2 and editing-loop waves. `07-decision-log.md` is the **index** and the tie-breaker on any doc conflict; each decision is a file in [`adr/`](adr/README.md) named by its number, so citations by number keep resolving |
...
| [15-metrics-and-instrumentation.md](15-metrics-and-instrumentation.md) | The north star (approved agent changes per user per week), the metric tree (activation T4 / habit / retention / guardrails), and the PostHog instrumentation plan with event dictionary and dashboards (wired in plan 36) |
...
| [agents/](agents/) | Per-repo configuration for the installed agent skills: issue tracker conventions (GitHub, fork-pinned), triage label vocabulary, and domain-doc consumer rules. Edit directly to change; referenced from `AGENTS.md` |
