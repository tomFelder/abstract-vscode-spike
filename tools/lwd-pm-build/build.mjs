...
// This directory lives at <repo>/build/lwd-pm-build, so the repo root is two levels up.
// LWD_REPO still overrides, for running the build against a checkout elsewhere.
const REPO = process.env.LWD_REPO || dirname(dirname(dirname(fileURLToPath(import.meta.url))));
const TARGET = join(REPO, 'src/vs/workbench/contrib/livingDocs/browser/prosemirrorBundle.ts');
