...
${String(isWordHtml)}
${String(hasHtmlTable)}
${String(convertHtmlTablesToDataMd)}
...
// Paste interception (issues #137/#139 Word normalisation, #138 tables). Capture phase so we run BEFORE
// ProseMirror's own clipboard handler. We intercept when the clipboard 'text/html' is EITHER a Word/Office
// payload (rebuild lists, drop nbsp spacers, resolve tracked changes, convert tables) OR any HTML that
// carries a <table> (Word or plain): a plain-HTML table is converted to <table data-md="..."> so PM parses
// it into a real table_block instead of flattening every cell into its own paragraph. A non-Word paste with
// no table falls through to PM untouched (inline fidelity is already good), as does a paste into a
// widget/textarea rather than the document. Fail-soft: if reading the clipboard or normalising throws, we do
// nothing and let the default paste proceed. pasteHTML feeds PM's paste pipeline without re-dispatching a
// paste event, so this never recurses.
root.addEventListener('paste', e => {
	...
	if (!word && !hasHtmlTable(html)) { return; }
	e.preventDefault(); e.stopPropagation();
	let cleaned;
	try { cleaned = word ? normalizeWordPasteHtml(html) : convertHtmlTablesToDataMd(html); } catch (err) { cleaned = html; }
