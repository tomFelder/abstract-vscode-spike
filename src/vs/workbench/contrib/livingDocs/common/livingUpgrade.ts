...
// that moment -- when the first source is bound (a frontmatter `sources:`/`context:` entry or a bind link).
// This module is the PURE, service-free statement of "has this document earned living?" and "does this
// project have anything living to speak of yet?", so the rule is unit-tested directly and every entry-path
// surface that decides whether to show living vocabulary reads from ONE place instead of re-deriving it
// inline. No DOM, no service, no I/O.

/**
 * The facts a single document exposes about whether it has crossed into "living". Mirrors the model's
 * `isLiving` derivation (`common/livingDocMarkdown.ts`: sources/context/binds). Kept as plain facts so
 * callers -- parser output, service state, or a summary -- can each supply them without this module reaching
 ...
 * link). A plain `.md` the user merely opened, typed into, and saved has none of these, so it stays plain --
 * no lock is owed and no living vocabulary is shown. This is the single predicate the entry path consults.
 */
export function docHasEarnedLiving(facts: ILivingUpgradeFacts): boolean {
	return facts.hasFrontmatterSources || facts.hasFrontmatterContext || facts.hasBindLinks;
}
