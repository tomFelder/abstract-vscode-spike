...
const OPENROUTER_MODELS = [
	// --- validated -----------------------------------------------------------------------------------
	{
		...
		notes: 'The founder-funded fallback since plan 35. Reliable on single-paragraph edits; weak on multi-'
			+ 'paragraph rewrites and on holding the JSON edit contract (a contributing cause of #303).',
	},
	...
	{ id: 'anthropic/claude-opus-4.8', label: 'Opus 4.8', validated: false, notes: 'Candidate: strongest expected on the structured-edit contract.' },
	{ id: 'anthropic/claude-sonnet-4.6', label: 'Sonnet 4.6', validated: false, notes: 'Candidate: cost/quality balance for everyday turns.' },
	{ id: 'openai/gpt-4.1', label: 'GPT-4.1', validated: false, notes: 'Candidate: the full-size sibling of the current default.' },
...
function listModels(opts) {
	...
	if (forced) {
		return [{ id: forced, label: 'Included model', default: true }];
	}
	...
	const base = (config && config.models)
		...
		: OPENROUTER_MODELS
			...
			.map(m => ({ id: m.id, label: m.label, default: m.default === true }));
	if (!base.length) {
		return [{ id: builtInDefaultId(), label: 'Included model', default: true }];
	}
